...
test.describe("Offline support", () => {
  test("Single tab — CRUD without reload", async ({ page }) => {
    ...
    await page.evaluate(() => window.__VAULTSYNC__.insert("crud-1", "test item"))
    await expect(page.locator("[data-id='crud-1']")).toBeVisible()
    await page.evaluate(() => window.__VAULTSYNC__.update("crud-1", { title: "updated" }))
    await expect(page.locator("[data-id='crud-1']")).toContainText("updated")
    await page.evaluate(() => window.__VAULTSYNC__.delete("crud-1"))
  ...
  test("Write offline, reconnect — 10 mutations synced", async ({ page }) => {
    ...
    for (let i = 0; i < 10; i++) {
      await page.evaluate((idx) =>
        window.__VAULTSYNC__.insert(`q-${idx}`, `queued ${idx}`), i)
    }
