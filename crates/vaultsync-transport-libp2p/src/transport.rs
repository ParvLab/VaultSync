...
pub struct LibP2pTransport {
    namespace: String,
    swarm: Swarm<VaultSyncBehaviour>,
...
impl LibP2pTransport {
    pub async fn new(
        ...
        let topic_str = format!("vaultsync/{}", namespace);
        ...
        loop {
            tokio::select! {
                event = swarm.select_next_some() => {
                    match event {
                        SwarmEvent::Behaviour(VaultSyncBehaviourEvent::Mdns(mdns::Event::Discovered(list))) => {
                            for (peer_id, addr) in list {
                                tracing::info!("P2P discovered peer: {} at {}", peer_id, addr);
                                swarm.behaviour_mut().gossipsub.add_explicit_peer(&peer_id);
                            }
                        }
                        SwarmEvent::Behaviour(VaultSyncBehaviourEvent::Mdns(mdns::Event::Expired(list))) => {
                            for (peer_id, addr) in list {
                                tracing::info!("P2P expired peer: {} at {}", peer_id, addr);
                                swarm.behaviour_mut().gossipsub.remove_explicit_peer(&peer_id);
                            }
                        }
                        SwarmEvent::Behaviour(VaultSyncBehaviourEvent::Gossipsub(gossipsub::Event::Message {
                    ...
                    match cmd {
                        Command::Broadcast(mutation) => {
                            if let Ok(bytes) = rmp_serde::to_vec(&mutation) {
                                let topic = gossipsub::IdentTopic::new(format!("vaultsync/{}", namespace));
