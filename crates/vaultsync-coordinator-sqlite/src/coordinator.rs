...
impl Coordinator for SQLiteCoordinator {
    ...
    async fn list_replicas(&self, namespace: &str) -> Result<Vec<ReplicaInfo>, CoordinatorError> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let rows = stmt
                .query_map(params![namespace_str], |row| {
            ...
            // 2. Delete mutations with sequence <= snap.sequence
            {
                ...
                for (doc_id, record_id, sequence) in snaps {
                    let deleted = del_stmt
                        .execute(params![ns, doc_id, record_id, sequence])
                        .map_err(|e| CoordinatorError::Internal(e.to_string()))?;
                    if deleted > 0 {
    ...
    async fn list_snapshots(
        ...
        tokio::task::spawn_blocking(move || {
            let conn_guard = conn
                ...
                .map_err(|e| CoordinatorError::Internal(e.to_string()))?;
            let mut stmt = conn_guard
                .prepare("SELECT bytes FROM snapshots WHERE namespace = ?1")
                .map_err(|e| CoordinatorError::Internal(e.to_string()))?;
...
mod tests {
    ...
    async fn test_sqlite_coordinator_snapshots() {
        ...
        let doc = vaultsync_core::crdt::document::CRDTDocument::new("doc-1", "rec-1", 1);
        let snap = vaultsync_core::crdt::snapshot::Snapshot::from_document(&doc, 42);
    ...
    async fn test_sqlite_coordinator_compaction() {
        ...
        let doc = vaultsync_core::crdt::document::CRDTDocument::new("doc-1", "rec-1", 1);
        let snap = vaultsync_core::crdt::snapshot::Snapshot::from_document(&doc, 1);
        coord.store_snapshot(ns, &snap).await.unwrap();
