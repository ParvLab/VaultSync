...
export function acquire_web_lock(lock_name, on_acquired, on_released) {
    let release_promise = new Promise((resolve) => {
        window._drift_releases = window._drift_releases || {};
        window._drift_releases[lock_name] = resolve;
    });
...
export function release_web_lock(lock_name) {
    if (window._drift_releases && window._drift_releases[lock_name]) {
        window._drift_releases[lock_name]();
        delete window._drift_releases[lock_name];
    }
}
"#)]
extern "C" {
...
impl LeaderElection {
    ...
    pub fn try_acquire(&self) -> Result<bool, DriftError> {
        ...
        #[cfg(target_arch = "wasm32")]
        {
            ...
            let name = format!("drift-{}-{}", self.namespace, self.unique_key);
