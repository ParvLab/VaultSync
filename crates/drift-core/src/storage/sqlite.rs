...
impl SQLiteStorage {
    pub fn new(path: &str) -> Result<Self, DriftError> {
    ...
    pub fn in_memory() -> Result<Self, DriftError> {
    ...
    fn init_tables(&self) -> Result<(), DriftError> {
...
impl Storage for SQLiteStorage {
    ...
    async fn mark_failed(&self, id: &str, _error_msg: &str) -> Result<(), DriftError> {
        let conn = self.lock();
        conn.execute(
        ...
        match rows.next()? {
            Some(row) => {
                ...
                Ok(Some(SyncState {
                    ...
                    schema_version: row.get::<_, i64>(8)? as u64,
                }))
            }
            None => Ok(None),
        }
