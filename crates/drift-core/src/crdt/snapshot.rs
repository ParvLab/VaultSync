...
impl Snapshot {
    ...
    pub fn encode(&self) -> Result<Vec<u8>, DriftError> {
        bincode::serialize(self)
            .map_err(|e| DriftError::Crdt(format!("failed to serialize snapshot: {e}")))
    }

    pub fn decode(bytes: &[u8]) -> Result<Self, DriftError> {
        let snap: Self = bincode::deserialize(bytes)
            .map_err(|e| DriftError::Crdt(format!("failed to deserialize snapshot: {e}")))?;
        Ok(snap)
    }
...
pub fn create_snapshot_with_meta(doc: &CRDTDocument, sequence: u64) -> Result<Vec<u8>, DriftError> {
...
pub fn load_snapshot(bytes: &[u8]) -> Result<CRDTDocument, DriftError> {
    ...
    if !snap.verify_checksum() {
        return Err(DriftError::Crdt("snapshot checksum verification failed".to_string()));
    }
    CRDTDocument::from_snapshot(&snap.bytes)
}

pub fn load_and_verify_snapshot(bytes: &[u8]) -> Result<(CRDTDocument, Snapshot), DriftError> {
    let snap = Snapshot::decode(bytes)?;
    if !snap.verify_checksum() {
        return Err(DriftError::Crdt("snapshot checksum verification failed".to_string()));
    }
...
mod tests {
    ...
    fn test_snapshot_roundtrip() {
        ...
        doc.set_field("name", crate::crdt::types::CrdtValue::String("drift".to_string()));
        ...
        assert_eq!(
            ...
            Some(crate::crdt::types::CrdtValue::String("drift".to_string()))
        );
