//! Test utilities: DriftFixture, SimulatedNetwork, DeterministicClock.
...
/// Pre-configured Drift test fixture with isolated storage, mock coordinator,
...
impl DriftFixture {
    ...
    pub async fn upload(&self) -> Result<usize, DriftError> {
    ...
    pub async fn download(&self) -> Result<usize, DriftError> {
        ...
        let mutations = self.coordinator
            ...
            .await
            .map_err(|e| DriftError::Coordinator(format!("{e:?}")))?;
    ...
    pub async fn sync(&self) -> Result<(), DriftError> {
        self.upload().await?;
        self.download().await?;
        Ok(())
    }
...
impl SimulatedReplica {
    ...
    pub async fn flush(&self) -> Result<usize, DriftError> {
        if !self.connected { return Ok(0); }
        self.fixture.upload().await
    }

    pub async fn pull(&self) -> Result<usize, DriftError> {
        if !self.connected { return Ok(0); }
...
impl SimulatedNetwork {
    ...
    pub fn add_replica(
        ...
        let fixture = DriftFixture::new(storage, self.coordinator.clone())
            .with_replica_id(id);
    ...
    /// Sync with timeout.
    pub async fn sync_all_with_timeout(&mut self, timeout: Duration) -> Result<(), DriftError> {
        tokio::time::timeout(timeout, async { self.sync_all().await })
            .await
            .map_err(|_| DriftError::Coordinator("sync_all timed out".to_string()))
    }
