...
impl SchemaRegistry {
    ...
    pub fn define(&mut self, doc_id: &str, schema: DocumentSchema) -> Result<(), DriftError> {
    ...
    pub fn validate_update(&self, doc_id: &str, field: &str, _value: &CrdtValue) -> Result<(), DriftError> {
        if let Some(schema) = self.schemas.get(doc_id) {
            if !schema.fields.iter().any(|f| f.name == field) {
                return Err(DriftError::Schema(format!("unknown field '{}' in doc '{}'", field, doc_id)));
            }
        }
        Ok(())
    }
