...
pub struct MigrationDefinition {
    pub version: String,
    pub checksum: String,
    pub apply_fn: Box<dyn Fn() -> Result<(), DriftError> + Send + Sync>,
    pub rollback_fn: Option<Box<dyn Fn() -> Result<(), DriftError> + Send + Sync>>,
}

impl MigrationDefinition {
    pub fn new(version: &str, apply_fn: Box<dyn Fn() -> Result<(), DriftError> + Send + Sync>) -> Self {
    ...
    pub fn with_rollback(mut self, rollback_fn: Box<dyn Fn() -> Result<(), DriftError> + Send + Sync>) -> Self {
