...
impl DriftMetrics {
    pub fn new() -> Self {
        ...
        let mutations_total = IntCounterVec::new(
            Opts::new("drift_mutations_total", "Total number of mutations attempted"),
            &["status", "namespace"]
        ).unwrap();
        let mutations_failed = IntCounterVec::new(
            Opts::new("drift_mutations_failed", "Total number of failed mutations"),
            &["namespace", "reason"]
        ).unwrap();
        let conflicts_total = IntCounterVec::new(
            Opts::new("drift_conflicts_total", "Total number of sync conflicts encountered"),
            &[]
        ).unwrap();
        let mutations_pending = IntGaugeVec::new(
            Opts::new("drift_mutations_pending", "Number of mutations pending sync"),
            &["namespace"]
        ).unwrap();
        let connection_status = IntGaugeVec::new(
            Opts::new("drift_connection_status", "Connection status (1=connected, 0=disconnected)"),
            &["namespace"]
        ).unwrap();
        let leader_status = IntGaugeVec::new(
            Opts::new("drift_leader_status", "Leader status (1=leader, 0=reader)"),
            &[]
        ).unwrap();
        let oplog_size = IntGaugeVec::new(
            Opts::new("drift_oplog_size", "Size of the oplog in entries"),
            &["namespace"]
        ).unwrap();
        let replica_count = IntGaugeVec::new(
            Opts::new("drift_replica_count", "Number of known replicas"),
            &["namespace"]
        ).unwrap();
        let doc_count = IntGaugeVec::new(
            Opts::new("drift_doc_count", "Number of documents"),
            &["doc_id"]
        ).unwrap();
        let sync_lag_ms = HistogramVec::new(
            HistogramOpts::new("drift_sync_lag_ms", "Sync lag in milliseconds")
                .buckets(vec![10.0, 50.0, 100.0, 500.0, 1000.0, 5000.0, 10000.0]),
            &["namespace"]
        ).unwrap();
        let download_lag_ms = HistogramVec::new(
            HistogramOpts::new("drift_download_lag_ms", "Download lag in milliseconds")
                .buckets(vec![10.0, 50.0, 100.0, 500.0, 1000.0, 5000.0, 10000.0]),
            &["namespace"]
        ).unwrap();
        let encryption_time_us = HistogramVec::new(
            HistogramOpts::new("drift_encryption_time_us", "Encryption/decryption time in microseconds")
                .buckets(vec![1.0, 5.0, 10.0, 50.0, 100.0, 500.0, 1000.0]),
            &["operation"]
        ).unwrap();
        let crdt_merge_time_us = HistogramVec::new(
            HistogramOpts::new("drift_crdt_merge_time_us", "CRDT merge time in microseconds")
...
#[cfg(feature = "telemetry")]
pub fn get_metrics() -> &'static DriftMetrics {
    METRICS.get_or_init(DriftMetrics::new)
}
