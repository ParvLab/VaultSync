...
use drift_core::e2ee::keyring::{KeyRing, E2eeEncryptor, E2eeDecryptor};
use drift_core::storage::sqlite::SQLiteStorage;
use drift_core::storage::traits::Storage;
use drift_core::oplog::entry::{OplogEntry, SyncStatus, MutationType};
...
fn bench_crdt_merge(c: &mut Criterion) {
    c.bench_function("crdt_merge_500_concurrent", |b| {
        b.iter_with_setup(|| {
            ...
            for i in 0..500 {
                updates_a.push(replica_a.set_field(&format!("alice-key-{}", i), CrdtValue::Number(i as f64)));
                updates_b.push(replica_b.set_field(&format!("bob-key-{}", i), CrdtValue::Number(i as f64)));
            }
...
fn bench_encrypt_decrypt(c: &mut Criterion) {
    ...
    let mut doc = CRDTDocument::new("doc-1", "rec-1", 0);
    doc.set_field("title", CrdtValue::String("benchmarking encrypt decrypt".to_string()));
...
fn bench_oplog_append(c: &mut Criterion) {
    ...
    c.bench_function("oplog_append_100", |b| {
        b.iter_with_setup(|| {
            ...
            let db_path = temp_dir.path().join("bench_append.db").to_string_lossy().to_string();
...
fn bench_oplog_read_pending(c: &mut Criterion) {
    ...
    c.bench_function("oplog_read_pending_100", |b| {
        b.iter_with_setup(|| {
            let temp_dir = tempfile::tempdir().unwrap();
            let db_path = temp_dir.path().join("bench_read.db").to_string_lossy().to_string();
            let storage = SQLiteStorage::new(&db_path).unwrap();
            rt.block_on(async {
                for i in 0..100 {
                    let entry = OplogEntry {
...
criterion_group!(benches, bench_crdt_insert, bench_crdt_merge, bench_encrypt_decrypt, bench_oplog_append, bench_oplog_read_pending);
