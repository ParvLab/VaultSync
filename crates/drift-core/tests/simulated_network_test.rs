...
async fn test_two_replica_basic_convergence() {
    ...
    net.replicas[0].write("todos", "t:1", s("title", "buy milk")).await.unwrap();
    net.sync_all().await;
    net.replicas[1].write("todos", "t:2", s("title", "get eggs")).await.unwrap();
    net.sync_all().await;
...
async fn test_three_replica_offline_convergence() {
    ...
    net.replicas[0].write("docs", "d:1", s("text", "hello")).await.unwrap();
    net.sync_all().await;

    net.replicas[1].disconnect();
    net.replicas[1].write("docs", "d:2", s("text", "from bob")).await.unwrap();

    net.replicas[2].disconnect();
    net.replicas[2].write("docs", "d:1", s("text", "hello world")).await.unwrap();
...
async fn test_50_offline_writes_converge_after_reconnect() {
    ...
    for i in 0..50u64 {
        net.replicas[0].write("items", &format!("item:{i}"), s("v", &format!("val_{i}"))).await.unwrap();
    }

    net.replicas[1].write("items", "item:99", s("v", "from_b")).await.unwrap();
    net.sync_all().await;
...
async fn test_network_partition_fault() {
    ...
    let fault = NetworkPartitionFault { target_id: "r1".to_string() };
    net.inject_fault(&fault);

    // r1 is disconnected; r2 can write
    net.replicas[1].write("data", "d:1", s("k", "v")).await.unwrap();
    net.sync_all().await;

    // r1 is still disconnected — should not have d:1 yet
    // (state is partitioned)

    let fault2 = NetworkPartitionFault { target_id: "r1".to_string() };
