...
use drift_core::sync::compaction::{CompactionEngine, CompactionConfig};

#[tokio::test]
async fn test_snapshot_compaction_reduces_oplog() {
    let temp_dir = tempfile::tempdir().unwrap();
    let db_path = temp_dir.path().join("compaction.db").to_string_lossy().to_string();
    ...
    let client = DriftClient::new_with_keyring(config.clone(), coordinator.clone(), keyring.clone())
        .await
        .unwrap();
    ...
    // We can extract/access the client storage using drift_core's internal structure if it's pub,
    ...
    let storage = drift_core::storage::sqlite::SQLiteStorage::new(&temp_dir.path().join("compaction.db").to_string_lossy()).unwrap();
    let comp_engine = CompactionEngine::new(Arc::new(storage), comp_config);

    // 4. Verify oplog synced entries count is high
    let active_docs = comp_engine.run_snapshot_compaction("compaction-ns").await.unwrap();
...
async fn test_tombstone_gc_removes_deleted_docs() {
    let temp_dir = tempfile::tempdir().unwrap();
    let db_path = temp_dir.path().join("tombstone.db").to_string_lossy().to_string();
    ...
    // 1. Insert a document
    let mut fields = HashMap::new();
    fields.insert("name".to_string(), CrdtValue::String("GC Target".to_string()));
    ...
    let storage = drift_core::storage::sqlite::SQLiteStorage::new(&temp_dir.path().join("tombstone.db").to_string_lossy()).unwrap();
    let mut comp_config = CompactionConfig::default();
...
async fn test_key_rotation_re_encrypts_oplog() {
    let temp_dir = tempfile::tempdir().unwrap();
    let db_path = temp_dir.path().join("rotation.db").to_string_lossy().to_string();
    ...
    let mut fields = HashMap::new();
    fields.insert("secret".to_string(), CrdtValue::String("encrypted content".to_string()));
    client.insert("doc-rot", "record-rot", fields).await.unwrap();
    ...
    assert_eq!(doc_val.get("secret").unwrap(), &CrdtValue::String("encrypted content".to_string()));
}

#[tokio::test]
async fn test_key_rotation_uses_random_nonces() {
    let temp_dir = tempfile::tempdir().unwrap();
    let db_path = temp_dir.path().join("nonces.db").to_string_lossy().to_string();
    ...
    let ciphertext1 = client.rotate_keys().await.map(|_| {
        let _enc = keyring.active_key();
        let enc_helper = drift_core::e2ee::keyring::E2eeEncryptor::new(keyring.clone());
        enc_helper.encrypt_symmetric(plaintext, "nonces-ns").unwrap()
    }).unwrap();
    ...
    let dec_helper = drift_core::e2ee::keyring::E2eeDecryptor::new(keyring.clone());
    let dec1 = dec_helper.decrypt_symmetric(&ciphertext1, "nonces-ns").unwrap();
    let dec2 = dec_helper.decrypt_symmetric(&ciphertext2, "nonces-ns").unwrap();
