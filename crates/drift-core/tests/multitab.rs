...
use drift_core::ipc::shared_memory::{SharedMemory, encode_sync_state, decode_sync_state};
use drift_core::sync::state::{SyncState, ConnectionStatus};
...
async fn test_single_leader_elected() {
    ...
    let mut config3 = DriftConfig::default();
    ...
    let client3 = DriftClient::new(config3).await.unwrap();
    tokio::time::sleep(Duration::from_millis(100)).await;
