...
use drift_core::schema::migration::MigrationDefinition;
...
async fn test_migration_failure_rollback() {
    ...
    let migration = MigrationDefinition::new("v1.0.0", Box::new(|| {
        Err(DriftError::Schema("migration failed intentionally".into()))
    }))
...
async fn test_concurrent_migration() {
    ...
    // Client A
    let mut config_a = DriftConfig::default();
    config_a.storage = StorageConfig::Sqlite { path: db_path.clone() };
    let client_a = DriftClient::new(config_a).await.unwrap();

    // Client B
    let mut config_b = DriftConfig::default();
    config_b.storage = StorageConfig::Sqlite { path: db_path.clone() };
    let client_b = DriftClient::new(config_b).await.unwrap();
