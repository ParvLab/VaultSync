...
pub fn run(args: InspectArgs) {
    ...
    if let Some(filename) = args.export_trace {
        ...
        match client
            .get(format!("http://127.0.0.1:{}/debug/vaultsync/traces", port))
            .send()
        {
            Ok(resp) => match resp.json::<Vec<Value>>() {
                Ok(spans) => {
                    ...
                    println!(
                        "{} Traces successfully exported to {}",
                        "✅".green(),
                        filename
                    );
                }
    ...
    if args.stream {
        ...
        println!(
            "{}",
            "Streaming real-time vaultsync spans...".yellow().bold()
        );
        loop {
                        ...
                        if let Some(id) = span.get("id").and_then(|v| v.as_u64()) {
                            if seen_ids.insert(id) {
                                let name = span
                                    ...
                                    .unwrap_or("unknown");
                                let timestamp =
                                    span.get("timestamp").and_then(|v| v.as_str()).unwrap_or("");
                                let fields =
                                    span.get("fields").cloned().unwrap_or(serde_json::json!({}));
                                println!(
                                    "[{}] {} - {} fields: {}",
                                    timestamp.blue(),
                                    name.green().bold(),
    ...
    if args.state {
        ...
        println!(
            "{}",
            "        VAULTSYNC REPLICA STATE MONITOR         "
                .green()
                .bold()
        );
        println!("{}", "=============================================".cyan());
        ...
        match client
            .get(format!("http://127.0.0.1:{}/debug/vaultsync/state", port))
            .send()
        {
                ...
                Ok(state) => {
                    ...
                    if let Some(m) = metrics {
                        ...
                        println!(
                            "{:<20} {}",
                            "Mutations Uploaded:",
                            m.get("mutations_uploaded").unwrap_or(&serde_json::json!(0))
                        );
                        println!(
                            "{:<20} {}",
                            "Mutations Downloaded:",
                            m.get("mutations_downloaded")
                                .unwrap_or(&serde_json::json!(0))
                        );
                        println!(
                            "{:<20} {}",
                            "Pending Mutations:",
                            m.get("pending_mutations").unwrap_or(&serde_json::json!(0))
                        );
                        println!(
                            "{:<20} {}",
                            "Replica Count:",
                            m.get("replica_count").unwrap_or(&serde_json::json!(0))
                        );
                        println!(
                            "{:<20} {}",
                            "Sync Errors:",
                            m.get("sync_errors").unwrap_or(&serde_json::json!(0))
                        );
                    }
                }
                Err(e) => eprintln!("Error decoding JSON response: {}", e),
        ...
        if let Ok(resp) = client
            .get(format!(
                "http://127.0.0.1:{}/debug/vaultsync/state/documents",
                port
            ))
            .send()
        {
        ...
        if let Ok(resp) = client
            .get(format!(
                "http://127.0.0.1:{}/debug/vaultsync/state/keys",
                port
            ))
            .send()
        {
        ...
        if let Ok(resp) = client
            .get(format!(
                "http://127.0.0.1:{}/debug/vaultsync/state/replicas",
                port
            ))
            .send()
        {
    ...
    println!(
        "VaultSync inspect on port {} (stream: {}, state: {})",
        port, args.stream, args.state
    );
}
