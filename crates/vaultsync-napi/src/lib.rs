...
pub struct NapiVaultSyncClient {
    ...
    subscription_handles:
        Mutex<HashMap<String, vaultsync_core::subscription::engine::SubscriptionHandle>>,
}
...
impl NapiVaultSyncClient {
    ...
    pub fn subscribe(&self, doc_id: String, callback: JsFunction) -> napi::Result<String> {
        let ts_fn: ThreadsafeFunction<(String, String)> = callback.create_threadsafe_function(
            0,
    ...
    pub async fn sync_status(&self) -> napi::Result<String> {
        ...
        let pending =
            self.client.pending_uploads().await.map_err(|e| {
                napi::Error::from_reason(format!("Pending uploads failed: {:?}", e))
            })?;
...
pub async fn create_client(
    ...
    let client = VaultSyncClient::connect(config)
        .await
