...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::info!("Starting Drift Coordinator Server with config: {:?}", config);

    // Initialize the pluggable coordinator backend
    let coordinator: Arc<dyn drift_core::coordinator::traits::Coordinator> = match config.backend.as_str() {
        "sqlite" => {
            tracing::info!("Initializing SQLite coordinator backend at path: {}", config.db_path);
            Arc::new(drift_coordinator_sqlite::coordinator::SQLiteCoordinator::new(&config.db_path))
        }
        "redis" => {
            tracing::info!("Initializing Redis coordinator backend at URL: {}", config.db_url);
            let redis_coord = drift_coordinator_redis::coordinator::RedisCoordinator::new(&config.db_url).await
                .map_err(|e| format!("Redis initialization error: {:?}", e))?;
            Arc::new(redis_coord)
        }
        "memory" | _ => {
            tracing::info!("Initializing In-Memory coordinator backend");
            Arc::new(drift_coordinator_memory::coordinator::InMemoryCoordinator::new())
        }
    };
