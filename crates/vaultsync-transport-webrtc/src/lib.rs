    ...
    async fn push(&self, _namespace: &str, _mutations: Vec<EncryptedMutation>) -> Result<Vec<SequenceId>, CoordinatorError> {
        Err(CoordinatorError::NotSupported(
            "WebRTC P2P transport is only supported in WASM (browser) targets. \
             For server-to-server P2P, use vaultsync-transport-libp2p instead.".into()
        ))
    }
...
impl PeerCoordinator {
    ...
    pub async fn initiate_connection(&self, peer_id: &str) -> Result<(), String> {
        ...
        guard.peers.insert(peer_id.to_string(), PeerConnectionState {
            pc: pc.clone(),
            dc: Some(dc.clone()),
        });
        ...
        let offer = wasm_bindgen_futures::JsFuture::from(promise_offer).await
            .map_err(|e| format!("Failed to create offer: {:?}", e))?;
            
        let offer_sdp = offer.dyn_into::<web_sys::RtcSessionDescription>()
        ...
        wasm_bindgen_futures::JsFuture::from(promise_local).await
    ...
    pub async fn handle_signaling_message(&self, sender_id: &str, signal_type: &str, data: &str) -> Result<(), String> {
        let inner = self.inner.clone();
        let mut guard = inner.lock().await;
        ...
        let pc = if let Some(state) = guard.peers.get(sender_id) {
            ...
            guard.peers.insert(sender_id.to_string(), PeerConnectionState {
                pc: pc.clone(),
                dc: None,
            });
            pc
        ...
        if signal_type == "offer" {
            ...
            let ans = wasm_bindgen_futures::JsFuture::from(promise_ans).await
                .map_err(|e| format!("Failed to create answer: {:?}", e))?;
                
            let ans_sdp = ans.dyn_into::<web_sys::RtcSessionDescription>()
                .map_err(|_| "Failed to cast to RtcSessionDescription".to_string())?;
            ...
            self.send_signal(sender_id, "answer", &ans_sdp.sdp()).await?;
    ...
    fn create_peer_connection(&self, inner: &mut WasmPeerCoordinatorInner, peer_id: &str) -> Result<web_sys::RtcPeerConnection, String> {
        ...
        let onicecandidate = Closure::wrap(Box::new(move |ev: web_sys::RtcPeerConnectionIceEvent| {
            if let Some(candidate) = ev.candidate() {
                let this = this_clone.clone();
                let peer_id = peer_id_str.clone();
                wasm_bindgen_futures::spawn_local(async move {
                    let _ = this.send_signal(&peer_id, "candidate", &candidate.candidate()).await;
                });
            }
    ...
    async fn send_signal(&self, target_id: &str, signal_type: &str, data: &str) -> Result<(), String> {
        let inner = self.inner.lock().await;
        if let Some(ref cb) = inner.signal_tx {
            cb(target_id.to_string(), signal_type.to_string(), data.to_string());
        }
        Ok(())
    }
}
