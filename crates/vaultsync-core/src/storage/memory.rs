...
impl Storage for InMemoryStorage {
    async fn insert_document(&self, doc_id: &str, record_id: &str, bytes: &[u8]) -> Result<(), VaultSyncError> {
        self.documents.write().map_err(|e| VaultSyncError::Storage(e.to_string()))?
    ...
    async fn get_document(&self, doc_id: &str, record_id: &str) -> Result<Option<Vec<u8>>, VaultSyncError> {
        Ok(self.documents.read().map_err(|e| VaultSyncError::Storage(e.to_string()))?
    ...
    async fn write_document_and_oplog(&self, doc_id: &str, record_id: &str, bytes: &[u8], entry: &OplogEntry) -> Result<(), VaultSyncError> {
        let mut docs = self.documents.write().map_err(|e| VaultSyncError::Storage(e.to_string()))?;
        let mut oplog = self.oplog.write().map_err(|e| VaultSyncError::Storage(e.to_string()))?;
    ...
    async fn list_documents(&self, doc_id: &str) -> Result<Vec<(String, Vec<u8>)>, VaultSyncError> {
        let docs = self.documents.read().map_err(|e| VaultSyncError::Storage(e.to_string()))?;
    ...
    async fn append_oplog(&self, entry: &OplogEntry) -> Result<(), VaultSyncError> {
        self.oplog.write().map_err(|e| VaultSyncError::Storage(e.to_string()))?.push(entry.clone());
        Ok(())
    }

    async fn read_pending_oplog(&self, namespace: &str, limit: usize) -> Result<Vec<OplogEntry>, VaultSyncError> {
        let oplog = self.oplog.read().map_err(|e| VaultSyncError::Storage(e.to_string()))?;
        Ok(oplog.iter()
    ...
    async fn delete_synced_before(
        ...
        let mut store = self.oplog.write().map_err(|e| VaultSyncError::Storage(e.to_string()))?;
