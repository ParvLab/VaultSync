...
pub struct InMemoryCoordinator {
    ...
    snapshots: Arc<
        RwLock<
            std::collections::HashMap<(String, String, String), crate::crdt::snapshot::Snapshot>,
        >,
    >,
...
impl Coordinator for InMemoryCoordinator {
    async fn push(
        ...
        let mut ops = self
            .ops
            .write()
            .map_err(|_| CoordinatorError::NotAvailable)?;
        for m in mutations {
    ...
    async fn register(
        ...
        let mut reps = self
            ...
            .map_err(|_| CoordinatorError::NotAvailable)?;
        reps.insert(
            (namespace.to_string(), info.replica_id.clone()),
            info.clone(),
        );
    ...
    async fn update_replica_key(
        ...
        let mut keys = self
            ...
            .map_err(|_| CoordinatorError::NotAvailable)?;
        keys.insert(
            (namespace.to_string(), replica_id.to_string()),
    ...
    async fn get_replica_key(
        ...
        let keys = self
            ...
            .map_err(|_| CoordinatorError::NotAvailable)?;
        Ok(keys
            .get(&(namespace.to_string(), replica_id.to_string()))
            .cloned())
    }
    ...
    async fn compact_oplog(
        ...
        let snaps = self
            ...
            .read()
            .map_err(|_| CoordinatorError::NotAvailable)?;
        let namespace_snaps: Vec<crate::crdt::snapshot::Snapshot> = snaps
            .iter()
            .filter(|((ns, _, _), _)| ns == namespace)
            .map(|(_, v)| v.clone())
            .collect();
        ...
        for snap in namespace_snaps {
            ...
            for ((ns, seq), pm) in ops.iter() {
                if ns == namespace
                    && pm.doc_id == snap.doc_id
                    && pm.record_id == snap.record_id
                    && *seq <= snap.sequence
                {
