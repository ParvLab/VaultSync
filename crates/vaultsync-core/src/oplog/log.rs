...
impl OpLog {
    ...
    pub async fn update_blob(&self, id: &str, new_blob: &[u8]) -> Result<(), VaultSyncError> {
    ...
    pub async fn mark_synced(&self, id: &str, sequence: u64) -> Result<(), VaultSyncError> {
        self.storage.mark_synced(id, sequence).await
    }

    pub async fn mark_failed(&self, id: &str, error_msg: &str) -> Result<(), VaultSyncError> {
