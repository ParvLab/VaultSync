...
use vaultsync_core::ipc::shared_memory::{decode_sync_state, encode_sync_state, SharedMemory};
use vaultsync_core::storage::traits::StorageConfig;
use vaultsync_core::sync::state::{ConnectionStatus, SyncState};
...
async fn test_single_leader_elected() {
    ...
    config3.storage = StorageConfig::Sqlite {
        path: db_path.clone(),
    };
    ...
    let leaders_count = [
        status1.leader_status,
        status2.leader_status,
        status3.leader_status,
    ]
    .iter()
    ...
    assert_eq!(
        leaders_count, 1,
        "Exactly one client must be elected leader"
    );
...
async fn test_leader_drop_promotes_new_leader() {
    ...
    assert_eq!(
        status2_after.leader_status,
        Some(true),
        "Client 2 should be promoted to leader after Client 1 is dropped"
    );
