...
async fn test_snapshot_compaction_reduces_oplog() {
    ...
    let client = VaultSyncClient::new_with_keyring(config.clone(), coordinator.clone(), keyring.clone())
        .await
        .unwrap();
    ...
    let storage = vaultsync_core::storage::sqlite::SQLiteStorage::new(&temp_dir.path().join("compaction.db").to_string_lossy()).unwrap();
...
async fn test_tombstone_gc_removes_deleted_docs() {
    ...
    let storage = vaultsync_core::storage::sqlite::SQLiteStorage::new(&temp_dir.path().join("tombstone.db").to_string_lossy()).unwrap();
    let mut comp_config = CompactionConfig::default();
