...
async fn test_schema_migration_runner() {
    ...
    let migration = vaultsync_core::schema::migration::MigrationDefinition::new(
        "v1.0.0",
        Box::new(move || {
    ...
    let migration_again = vaultsync_core::schema::migration::MigrationDefinition::new(
        "v1.0.0",
        Box::new(move || {
...
async fn test_two_replicas_independent_keyrings_sync() {
    ...
    let kr_bob = vaultsync_core::e2ee::keyring::KeyRing::from_key(alice_key);
