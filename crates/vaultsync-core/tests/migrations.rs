...
async fn test_migration_runs_once() {
    ...
    // Define migration
    let migration1 = MigrationDefinition::new(
        "v1.0.0",
        Box::new(move || {
    ...
    let migration2 = MigrationDefinition::new(
        "v1.0.0",
        Box::new(move || {
...
async fn test_migration_ordering() {
    ...
    let mig1 = MigrationDefinition::new(
        "v1.0.0",
        Box::new(move || {
    ...
    let mig2 = MigrationDefinition::new(
        "v2.0.0",
        Box::new(move || {
...
async fn test_migration_failure_rollback() {
    ...
    let migration = MigrationDefinition::new(
        "v1.0.0",
        Box::new(|| {
            Err(VaultSyncError::Schema(
                "migration failed intentionally".into(),
            ))
        }),
    )
    ...
    let migration_success = MigrationDefinition::new(
        "v1.0.0",
        Box::new(move || {
    ...
    assert!(
        run_success.load(Ordering::SeqCst),
        "Migration should run successfully because failed attempt did not persist record"
    );
}
...
async fn test_concurrent_migration() {
    ...
    let db_path = temp_dir
        .path()
        .join("mig_concurrent.db")
        .to_string_lossy()
        .to_string();

    // Client A
    let mut config_a = VaultSyncConfig::default();
    config_a.storage = StorageConfig::Sqlite {
        path: db_path.clone(),
    };
    ...
    config_b.storage = StorageConfig::Sqlite {
        path: db_path.clone(),
    };
    ...
    let mig_a = MigrationDefinition::new(
        "v1.0.0",
        Box::new(move || {
    ...
    let mig_b = MigrationDefinition::new(
        "v1.0.0",
        Box::new(move || {
...
async fn test_migration_auto_execution() {
    ...
    let client = VaultSyncClient::new_with_storage_and_migrations(
        config,
        coordinator,
        keyring,
        storage,
