...
use vaultsync_core::coordinator::traits::{EncryptedMutation, PendingMutation, ReplicaInfo};
                ...
                if let Ok(n) = stream.read(&mut buf).await {
                    ...
                    if request.contains("POST ") && request.contains("/push") {
                        if let Some(body_start) = request.find("\r\n\r\n") {
                            ...
                            if let Ok(muts_list) = serde_json::from_str::<Vec<EncryptedMutation>>(
                                body.trim_end_matches('\0'),
                            ) {
                                let seqs = {
                        ...
                        let filtered = {
                            ...
                            let filtered: Vec<PendingMutation> = db
                                .iter()
                        ...
                        if let Some(body_start) = request.find("\r\n\r\n") {
                            let body = &request[body_start + 4..];
                            if let Ok(info) =
                                serde_json::from_str::<ReplicaInfo>(body.trim_end_matches('\0'))
                            {
