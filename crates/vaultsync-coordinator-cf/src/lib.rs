...
impl DurableObject for NamespaceDurableObject {
    ...
    async fn websocket_message(
        ...
        let (msg_type, payload) = match vaultsync_core::coordinator::ws_proto::decode_frame(&bin) {
            Ok(res) => res,
            Err(_) => return Ok(()),
        };
        ...
        match msg_type {
            vaultsync_core::coordinator::ws_proto::MSG_AUTH => {
                ...
                let ack = vaultsync_core::coordinator::ws_proto::AuthAckPayload {
                    status: "ok".to_string(),
                    error: None,
            ...
            vaultsync_core::coordinator::ws_proto::MSG_NAMESPACE_ADD => {
                ...
                for snap in available_snapshots {
                    let snap_payload = vaultsync_core::coordinator::ws_proto::SnapshotPayload {
                        doc_id: snap.doc_id,
