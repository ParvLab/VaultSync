...
pub fn run(args: InspectArgs) {
    ...
    if let Some(filename) = args.export_trace {
        ...
        match client.get(format!("http://127.0.0.1:{}/debug/drift/traces", port)).send() {
            Ok(resp) => {
    ...
    if args.stream {
        ...
        println!("{}", "Streaming real-time drift spans...".yellow().bold());
        loop {
            if let Ok(resp) = client.get(format!("http://127.0.0.1:{}/debug/drift/traces", port)).send() {
    ...
    if args.state {
        let client = reqwest::blocking::Client::new();
        println!("{}", "=============================================".cyan());
        println!("{}", "        DRIFT REPLICA STATE MONITOR         ".green().bold());
        println!("{}", "=============================================".cyan());

        match client.get(format!("http://127.0.0.1:{}/debug/drift/state", port)).send() {
            Ok(resp) => {
        ...
        // Fetch documents
        if let Ok(resp) = client.get(format!("http://127.0.0.1:{}/debug/drift/state/documents", port)).send() {
        ...
        // Fetch keys
        if let Ok(resp) = client.get(format!("http://127.0.0.1:{}/debug/drift/state/keys", port)).send() {
        ...
        // Fetch replicas
        if let Ok(resp) = client.get(format!("http://127.0.0.1:{}/debug/drift/state/replicas", port)).send() {
    ...
    println!("Drift inspect on port {} (stream: {}, state: {})", port, args.stream, args.state);
}
