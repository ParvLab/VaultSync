# Drift — End-to-End Implementation Plan
...
│
├── docs/
│   ├── drift-spec.md                   # System specification (complete)
│   ├── drift-testing-spec.md           # Testing specification (complete)
│   └── drift-implementation-plan.md    # This file
│
├── crates/
│   ├── drift-core/                     # Core Rust library
│   │   ├── Cargo.toml
│   │   └── src/
│   │       ├── lib.rs
│   │       ├── crdt/
...
│   │       ├── client.rs               # DriftClient: public API
│   │       ├── config.rs               # Configuration structs
│   │       ├── error.rs                # Error types
│   │       └── test_utils.rs           # TestFixture, SimulatedNetwork
│   │
│   ├── drift-wasm/                     # WASM bindings (wasm-pack)
│   │   ├── Cargo.toml
│   │   └── src/
│   │       ├── lib.rs
│   │       ├── client.rs               # WASM-exposed DriftClient
...
│   │
│   ├── drift-coordinator-postgres/     # Postgres coordinator package
│   │   ├── Cargo.toml
│   │   └── src/lib.rs
│   │
│   ├── drift-coordinator-redis/        # Redis coordinator package
│   │   ├── Cargo.toml
│   │   └── src/lib.rs
│   │
│   ├── drift-coordinator-sqlite/       # Embedded SQLite coordinator
│   │   ├── Cargo.toml
│   │   └── src/lib.rs
│   │
│   ├── drift-coordinator-memory/       # In-memory coordinator (testing)
│   │   ├── Cargo.toml
│   │   └── src/lib.rs
│   │
│   ├── drift-cli/                      # CLI tools
...
│   │   └── src/
│   │       ├── main.rs
│   │       ├── inspect.rs              # `drift inspect` — attach to running process
│   │       ├── replay.rs               # `drift replay` — replay trace file
│   │       └── bench.rs                # `drift bench` — run benchmarks manually
│   │
│   └── drift-fuzz/                     # Fuzz targets (cargo fuzz)
│       ├── Cargo.toml
│       └── fuzz_targets/
│           ├── crdt_merge.rs
│           ├── e2ee_ciphertext.rs
...
│
├── packages/
│   ├── web/                            # @drift/web — Browser SDK
│   │   ├── package.json
│   │   ├── tsconfig.json
│   │   ├── src/
│   │   │   ├── index.ts
│   │   │   ├── drift.ts               # Drift class (initialization)
...
│   │   ├── __tests__/
│   │   └── wasm/                      # Compiled drift-wasm output
│   │
│   ├── node/                           # @drift/node — Node.js SDK
│   │   ├── package.json
│   │   ├── tsconfig.json
│   │   ├── src/
│   │   │   ├── index.ts
...
│   │   └── __tests__/
│   │
│   ├── react/                          # @drift/react — React hooks
│   │   ├── package.json
│   │   ├── tsconfig.json
│   │   ├── src/
│   │   │   ├── index.ts
│   │   │   ├── provider.tsx            # DriftProvider context
│   │   │   ├── hooks.ts               # useDriftQuery, useDriftOne, useDriftSync
│   │   │   ├── mutations.ts           # useDriftMutations
│   │   │   └── sync-indicator.tsx      # SyncIndicator component
│   │   └── __tests__/
│   │
│   ├── coordinator-postgres/           # @drift/coordinator-postgres
│   │   ├── package.json
│   │   ├── tsconfig.json
│   │   └── src/
│   │       ├── index.ts
...
│   │
│   ├── coordinator-redis/              # @drift/coordinator-redis
│   │   ├── package.json
│   │   └── src/
│   │       ├── index.ts
│   │       └── redis-coordinator.ts
│   │
│   ├── coordinator-supabase/           # @drift/coordinator-supabase
│   │   ├── package.json
│   │   └── src/
│   │       ├── index.ts
│   │       └── supabase-coordinator.ts
│   │
│   └── coordinator-cloudflare/         # @drift/coordinator-cloudflare
│       ├── package.json
│       ├── tsconfig.json
│       ├── src/
│       │   ├── index.ts
...
```
          ...
          │                │                │
   ┌──────▼──────┐  ┌──────▼──────┐  ┌──────▼──────┐
   │drift-wasm   │  │drift-coord- │  │drift-coord- │
   │             │  │postgres     │  │redis        │
   │ deps:       │  │             │  │             │
...
| **P0.3** Create drift-core crate | `crates/drift-core/Cargo.toml` | Core dependencies: `yrs = "0.19"`, `libsodium-sys`, `tokio`, `serde`, `tracing`, `opentelemetry`, `rusqlite` |
| **P0.4** Create drift-wasm crate | `crates/drift-wasm/Cargo.toml` | Add `wasm-bindgen`, `wasm-pack` build config |
| **P0.5** Create coordinator crates | `crates/drift-coordinator-*/Cargo.toml` | Postgres (`sqlx`), Redis (`redis-rs`), SQLite (`rusqlite`), Memory (none) |
| **P0.6** Create drift-cli crate | `crates/drift-cli/Cargo.toml` | `clap` for CLI args, `tokio` for async |
| **P0.7** Create drift-fuzz crate | `crates/drift-fuzz/Cargo.toml` | `cargo-fuzz` config, libfuzzer-sys dependency |
...
#### Step 1.1: CRDT Document Layer (`crates/drift-core/src/crdt/`)
...
#### Step 1.2: E2EE Layer (`crates/drift-core/src/e2ee/`)
...
#### Step 1.3: Storage Trait + SQLite (`crates/drift-core/src/storage/`)
...
#### Step 1.4: OpLog (`crates/drift-core/src/oplog/`)
...
#### Step 1.5: Schema + Migrations (`crates/drift-core/src/schema/`)
...
#### Step 1.6: Subscriptions (`crates/drift-core/src/subscription/`)
...
#### Step 1.7: Sync Engine (`crates/drift-core/src/sync/`)
...
#### Step 1.8: Coordinator Trait (`crates/drift-core/src/coordinator/`)
...
#### Step 1.9: Client API (`crates/drift-core/src/`)
...
- Complete `drift-core` crate with all modules
...
### Goal
Compile `drift-core` to WASM for browser use. Implement OPFS and IndexedDB storage backends. Expose DriftClient via TypeScript bindings.

### Tasks

| Task | File(s) | Details |
|---|---|---|
| **P2.1** WASM storage trait | `crates/drift-wasm/src/storage.rs` | Implement Storage trait for OPFS (via `send_sync`) and IndexedDB (via web-sys). OPFS: open SQLite via WASM in Web Worker, use synchronous access. IndexedDB: store Yrs snapshots as IDB records |
| **P2.2** WASM IPC | `crates/drift-wasm/src/ipc.rs` | Wraps `BroadcastChannel` for cross-tab messaging, `navigator.locks` for leader election fallback |
| **P2.3** WASM E2EE | `crates/drift-wasm/src/e2ee.rs` | Maps `Encrypt`/`Decrypt` to WebCrypto `subtle.encrypt`/`subtle.decrypt` (X25519 not natively available in all browsers — fallback to WASM-compiled libsodium) |
| **P2.4** WASM transport | `crates/drift-wasm/src/transport.rs` | WebSocket via `web_sys::WebSocket`. HTTP/SSE fallback for coordinators that don't support WS |
| **P2.5** WASM client bindings | `crates/drift-wasm/src/client.rs` | #[wasm_bindgen] annotated wrapper: exposes `DriftClient` methods to JS |
| **P2.6** TypeScript types | `packages/web/src/` | Manual TypeScript type definitions that match the WASM exported API |
| **P2.7** Storage detection | `packages/web/src/drift.ts` | `Drift.detectStorage()`: try OPFS first, fallback to IndexedDB |

### WASM Build Configuration

```toml
# crates/drift-wasm/Cargo.toml
[package]
name = "drift-wasm"
version = "0.1.0"
edition = "2024"
...
- WASM-compiled `drift-wasm` (`.wasm` + JS glue)
...
| Task | File(s) | Details |
|---|---|---|
| **P3.1** File lock (native) | `crates/drift-core/src/ipc/leader_election.rs` | On Linux/macOS: `flock(fd, LOCK_EX | LOCK_NB)`. On Windows: `CreateMutex` via `winapi`. Backed by SQLite WAL lock file |
| **P3.2** BroadcastChannel (browser) | `crates/drift-wasm/src/ipc.rs` | `web_sys::BroadcastChannel` with channel name = namespace. Messages: heartbeats, leader announcements, state change notifications |
| **P3.3** Shared memory (native) | `crates/drift-core/src/ipc/shared_memory.rs` | Mmap shared file. Layout: header (leader_id, heartbeat, generation) + Yrs doc snapshot + ring buffer |
| **P3.4** SharedArrayBuffer (browser) | `crates/drift-wasm/src/ipc.rs` | `SharedArrayBuffer` (requires COOP/COEP headers). Atomics-based synchronization |
| **P3.5** Heartbeat | `crates/drift-core/src/ipc/heartbeat.rs` | Leader writes timestamp + sequence every 1s. Readers check every 500ms |
| **P3.6** Crash recovery | `crates/drift-core/src/ipc/crash_recovery.rs` | On leader promotion: read ring buffer, replay uncommitted Yrs updates, update sync_state |
| **P3.7** Leader election integration | `crates/drift-core/src/client.rs` | On `DriftClient::initialize()`: attempt leader election. If elected, bind write lock. If reader, route writes to leader via IPC |
...
### PostgresCoordinator (`crates/drift-coordinator-postgres/`)
...
impl Coordinator for PostgresCoordinator {
    async fn push(&self, namespace: &str, mutations: Vec<EncryptedMutation>) -> Result<Vec<SequenceId>> {
        // INSERT INTO drift_coordinator_mutations ... RETURNING sequence
    ...
    async fn pull(&self, namespace: &str, after: SequenceId, limit: usize) -> Result<Vec<PendingMutation>> {
        // SELECT * FROM drift_coordinator_mutations WHERE namespace=$1 AND sequence > $2 ORDER BY sequence LIMIT $3
    }

    async fn subscribe(&self, namespace: &str, from_sequence: SequenceId) -> Result<Box<dyn Stream<Item = PendingMutation>>> {
        // LISTEN drift_channel_{namespace}
        // On NOTIFY: pull from from_sequence
    }

    async fn register(&self, namespace: &str, info: ReplicaInfo) -> Result<()> {
        // UPSERT INTO drift_coordinator_replicas
    }

    async fn heartbeat(&self, namespace: &str, replica_id: &str) -> Result<()> {
        // UPDATE drift_coordinator_replicas SET last_heartbeat = NOW() WHERE id = $1
    }
}
```

### RedisCoordinator (`crates/drift-coordinator-redis/`)
...
// - push: XADD drift:mutes:{namespace} * sequence <auto> id ... encrypted ...
// - pull: XRANGE drift:mutes:{namespace} {after_sequence} +
// - subscribe: XREAD BLOCK 0 STREAMS drift:mutes:{namespace} {after_sequence}
// - register: HSET drift:replicas:{namespace} {replica_id} {json}
// - heartbeat: EXPIRE drift:replicas:{namespace}:{replica_id} 30
```
...
```
@drift/web (browser)
  ├── Drift class (facade over WASM exported functions)
...
@drift/node (server)
  ├── Drift class (facade over native Rust binary via FFI)
  ├── SQLiteStorage (better-sqlite3)
  └── Same CRUD/subscription API as @drift/web

@drift/react
  ├── DriftProvider (React context)
  ├── useDriftQuery (auto-subscribe, re-render on change)
  ├── useDriftOne (single record subscription)
  ├── useDriftMutations (insert, update, delete)
  ├── useDriftSync (sync status hook)
  └── SyncIndicator (pre-built component)
```

### Web SDK Core (`packages/web/src/drift.ts`)

```typescript
import init, { DriftClient } from "../wasm/drift_wasm.js"

export interface DriftWebConfig {
...
export class Drift {
  ...
  static async create(config: DriftWebConfig): Promise<Drift> {
    // 1. Load WASM module
    await init()
    ...
    return new Drift(client, config)
  }
...
export class Collection<T> {
  constructor(private name: string, private client: DriftClient) {}
...
import { DriftContext } from "./provider"

export function useDriftQuery<T>(
...
export function useDriftSync() {
  ...
  useEffect(() => {
    const unsub = drift.subscribe("sync:status", setStatus)
    return unsub
  }, [drift])
...
```typescript
export function defineSchema(drift: Drift, schema: SchemaDefinition) {
  // Convert TS schema to Rust-compatible JSON
  // Call drift.client.defineSchema(docId, schemaJson)
}
...
- `@drift/web` package loadable in browser
- `@drift/node` package loadable in Node.js
- `@drift/react` with `DriftProvider`, `useDriftQuery`, `useDriftOne`, `useDriftSync`
...
### Goal
Implement the full test suite defined in `drift-testing-spec.md`. This phase runs parallel to P1–P5 but culminates here.

### Tasks

| Task | Test Type | Files | Details |
|---|---|---|---|
| **P6.1** CRDT property tests | Unit | `crates/drift-core/src/crdt/*.rs` (test modules) | proptest convergence, commutativity, associativity, idempotence |
| **P6.2** E2EE KAT tests | Unit | `crates/drift-core/src/e2ee/kat.rs` | Known-answer vectors from libsodium spec |
| **P6.3** Storage conformance | Integration | `crates/drift-core/src/storage/tests/` | Same test suite runs against SQLite, InMemory, RocksDB |
| **P6.4** Sync cycle integration | Integration | `crates/drift-core/tests/sync_cycle.rs` | Write→encrypt→upload→coord→download→decrypt→merge→subscribe |
| **P6.5** Offline→online | Integration | `crates/drift-core/tests/offline_reconnect.rs` | Write offline, reconnect, sync, verify convergence |
| **P6.6** Multi-tab | Integration | `crates/drift-core/tests/multitab.rs` | Multi-process leader election, crash recovery |
| **P6.7** Coordinator conformance | Integration | `crates/drift-core/tests/coordinator_conformance.rs` | Macro-generated tests per coordinator |
| **P6.8** Chaos tests | Chaos | `crates/drift-core/tests/chaos/` | toxiproxy network faults, SIGKILL crash, clock skew |
| **P6.9** Migration tests | Integration | `crates/drift-core/tests/migrations.rs` | Schema migration forward/backward, concurrent migration |
| **P6.10** Benchmarks | Performance | `crates/drift-core/benches/` | Criterion benchmarks for all perf targets |
| **P6.11** Fuzz targets | Fuzz | `crates/drift-fuzz/fuzz_targets/` | 6 cargo-fuzz targets |
...
### Goal
Build CLI tools (`drift inspect`, `drift replay`) and complete OpenTelemetry integration.

### CLI (`crates/drift-cli/`)

```rust
// drift-inspect: attach to running Drift process
// Usage: drift inspect --pid <pid> [--stream] [--state]
...
**`inspect.rs`** — Attaches to running Drift process via:
...
```rust
// crates/drift-core/src/telemetry/tracing.rs
...
```rust
// crates/drift-core/src/telemetry/debug.rs
...
impl DebugApi {
    pub fn new(client: Arc<DriftClient>) -> Self;

    pub async fn serve(&self, port: u16) -> Result<()> {
        let app = axum::Router::new()
            .route("/debug/drift/state", axum::routing::get(self.get_state))
            .route("/debug/drift/state/oplog", axum::routing::get(self.get_oplog))
            .route("/debug/drift/state/documents", axum::routing::get(self.get_documents))
            .route("/debug/drift/state/keys", axum::routing::get(self.get_keys))
            .route("/debug/drift/state/replicas", axum::routing::get(self.get_replicas))
            .route("/debug/drift/metrics", axum::routing::get(self.get_metrics))
            .route("/debug/drift/leader", axum::routing::get(self.get_leader))
            .route("/debug/drift/force-sync", axum::routing::post(self.force_sync))
            .route("/debug/drift/force-election", axum::routing::post(self.force_election));
...
- `drift inspect` CLI (attach to process, stream ops, dump state)
- `drift replay` CLI (reproduce sync scenarios from traces)
...
coordinator: postgres://localhost:5432/drift_loadtest
...
```rust
// crates/drift-core/src/transport/webrtc.rs (feature-gated)
...
```rust
// crates/drift-core/src/transport/mesh.rs (feature-gated)

pub struct MeshTransport {
    swarm: libp2p::Swarm<DriftBehaviour>,
...
|---|---|---|
| **Go** | `sdks/go/drift.go` | CGo bindings to `drift-core` compiled as C library. Expose `DriftClient`, `Schema`, `Subscription` types |
| **Swift** | `sdks/swift/Drift.swift` | Swift Package Manager. Wraps `drift-core` via C interop. Native async/await API |
| **Kotlin** | `sdks/kotlin/Drift.kt` | JNI bindings to `drift-core`. Expose as Kotlin coroutines |

Each SDK repeats the same pattern:
1. Compile `drift-core` as a C dynamic library (`cdylib`)
...
|---|---|---|
| `drift-core` (Rust crate) | crates.io | `cargo publish` in release pipeline |
| `drift-wasm` (WASM) | npm (`@drift/web`) | `wasm-pack publish` |
| `drift-coordinator-*` | crates.io | `cargo publish` per crate |
| `@drift/web` | npm | `npm publish` |
| `@drift/node` | npm | `npm publish` |
| `@drift/react` | npm | `npm publish` |
| `@drift/coordinator-*` | npm | `npm publish` per package |
| `drift-swift` | SPM | Git tag |
| `drift-android` | Maven Central | Gradle publish |
...
jobs:
  ...
  publish-rust:
    ...
    steps:
      ...
      - run: cargo publish -p drift-core
      - run: cargo publish -p drift-coordinator-postgres
      - run: cargo publish -p drift-coordinator-redis
      - run: cargo publish -p drift-coordinator-sqlite
      - run: cargo publish -p drift-coordinator-memory
      - run: cargo publish -p drift-cli

  publish-wasm:
    needs: [verify]
    runs-on: ubuntu-latest
    steps:
      - run: wasm-pack build crates/drift-wasm --target web
...
|---|---|---|---|
| **Yrs integration issues** (API changes, performance bugs) | Medium | High | Pin Yrs version. Run Yrs test suite as part of Drift CI. Contribute upstream fixes |
