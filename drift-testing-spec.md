# Drift — Testing & Verification Specification
...
Drift's testing strategy is built on five non-negotiable principles:
...
Network failures, process crashes, disk corruption, and clock skew are not edge cases — they are normal operating conditions in local-first sync. Drift must survive all of them without data loss.
...
|---|---|---|
| PostgresCoordinator | `postgres://localhost:5432/drift_test` | Requires Docker |
...
E2E tests simulate real user interactions with the full Drift stack including browser or native runtime.
...
Drift provides a `test::SimulatedNetwork` harness for multi-device scenarios:
...
| Node.js | Jest + Supertest | Server Drift instance, sync between two server processes |
...
Drift uses the Yrs CRDT library, which has its own extensive test suite. Drift's CRDT tests should:

1. **Reuse Yrs's known-answer tests**: Yrs publishes test vectors for merge scenarios
2. **Extend beyond Yrs's tests**: Add Drift-specific scenarios (E2EE + CRDT, schema migrations + CRDT)
3. **Test Yrs at Drift's scale**: 10,000 concurrent mutations across 100 replicas
...
Each encryption operation has a KAT that verifies Drift's implementation matches the libsodium specification:
...
async fn test_leader_crash_recovery() {
    ...
    let leader = spawn_drift_process(&dir, "leader");
    leader.wait_for_leader().await;

    // Spawn reader process
    let reader = spawn_drift_process(&dir, "reader");
    ...
    let latecomer = spawn_drift_process(&dir, "latecomer");
...
1. **Inject real faults**: Drift's chaos tests use `toxiproxy` (network fault injection) and OS-level process control (SIGKILL, SIGSTOP), not simulated timeouts
...
| **Corrupt oplog entry** | Random byte modified in oplog file | 30s | Drift detects corruption, logs error, replays from last good entry |
...
The primary test fixture for integration tests. Provides pre-configured Drift instances with isolated storage, mock coordinator, and deterministic clock.
...
impl DriftFixture {
    ...
    pub fn assert_converges_with(&self, other: &DriftFixture) { ... }
}
```
...
- name: Enforce thresholds
  run: |
    ...
    assert_coverage "drift-core/src/crdt" 100 100
    assert_coverage "drift-core/src/e2ee" 100 100
    assert_coverage "drift-core/src/ipc"  100 100
    # Overall must meet targets
    assert_coverage "drift-core" 94 91
```

### 16.3 What Is Excluded from Coverage

- Integration test helpers and harness code
- CLI tools (`drift inspect`, `drift replay`)
